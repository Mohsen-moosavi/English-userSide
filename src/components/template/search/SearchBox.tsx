...
import { changePaginatorChangerFlag, setArticleOffset, setArticles, setBookOffset, setBooks, setCourseOffset, setCourses, setSearchword } from '@/redux/slice/search/searchSlice'
import React from 'react'

function SearchBox() {
    const { searchWord, bookLoaded,courseLoaded,articleLoaded } = useAppSelector(state => state.search)
    ...
    return (
        <form className='relative max-md:w-full' onSubmit={(e) => searchFormSubmitHandler(e)}>
            <input
                ...
                placeholder="جستجو..."
                value={searchWord}
                disabled={!bookLoaded || !courseLoaded}
