...
type PageProps = {
    ...
    courseId: number,
    courseName: string,
    defaultSessions: { isFree: boolean, time: string, id: number, name: string }[]
}

function SessionList({ sessionCount, defaultSessions, courseSlug, courseId, courseName }: PageProps) {
                    ...
                    {defaultSessions.map((session, index) => (
                        <div className={`py-2 ${index === 0 ? '' : 'border-t border-solid border-custom-dark-blue'}`} key={index}>
                            {(session.isFree || userCourses?.includes(courseId)) ? (
                                <SessionLink courseId={courseId} sessionId={session.id} number={index + 1} sessionTime={session.time} sessionName={session.name} />
                            ) : (
