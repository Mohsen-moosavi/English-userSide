...
function SessionList({ sessionCount , courseSlug, courseId }: { sessionCount: number, courseSlug:string, courseId:number }) {
                    ...
                    <div className='flex justify-center'>
                        ...
                        {moreSessions?.map((session, index) => (
                                                    <div className={`py-2 ${index === 0 ? '' : 'border-t border-solid border-custom-dark-blue'}`} key={index}>
                                                        {session.isFree ? (
                                                            <Link href={`/courses/${courseSlug}/${session.id}`} className="flex items-center gap-x-1 sm:gap-x-3 text-custom-dark-blue max-sm:text-[12px] hover:bg-sky-400/10">
