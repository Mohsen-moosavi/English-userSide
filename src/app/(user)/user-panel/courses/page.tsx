...
import { getUserCoursesThunk } from '@/redux/slice/user/userThunks'
...
function page() {
  ...
  async function getData() {
    const dispatchResult = await dispatch(getUserCoursesThunk())
    if ((dispatchResult.meta.requestStatus = 'fulfilled') && !dispatchResult.type.endsWith('rejected')) {
      setCourses(dispatchResult.payload.data.courses)
    }
    setLoading(false)
  }
  ...
  return (
    <>
      <h1 className='text-center text-custom-dark-blue font-bold md:hidden mb-6'>دوره ها</h1>
      {loading ? (
        ...
        (
          <>
            {courses.length ? (
              ...
              <div className="h-[300px] w-full flex items-center justify-center">
                <span className='text-red-400 text-md'>شما هنوز در دوره ای ثبت نام نکرده اید!</span>
              </div>
            )}
          </>
